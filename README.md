...
## Project Structure
- `seismic_bumps_analysis.ipynb`: the narrated, self-contained end-to-end analysis (start here).
- `streamlit_app.py`: the interactive demo app, with its two helper modules
  `scoring.py` and `risk_levels.py`.
- `data/`: the bundled raw dataset (`data/raw/seismic-bumps.arff`).
- `artifacts/`: saved metrics, experiment results, and the frozen model bundle the app serves.
